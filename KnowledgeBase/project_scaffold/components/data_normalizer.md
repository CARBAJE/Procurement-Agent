...
## State Machine: `procurement_status` 
...
## Audit Trail — FK Chain

The Data Normalizer builds a **progressive audit tree** in the database. Each node is a DB row created by a specific `_persist()` call from the Orchestrator, and each child row holds the UUID of its parent as a Foreign Key.

```
procurement_requests  (uuid-A)        ← POST /normalize/request
                                         Orchestrator: user input
    └── parsed_intents  (uuid-B)      ← POST /normalize/intent
    │                                    Microservice: Intention Parser (8001)
          └── beckn_intents  (uuid-C) ← POST /normalize/intent (same call, 2 rows)
                                         Intent structured in Beckn format
                └── discovery_queries  (uuid-D)      ← POST /normalize/discovery
                │                                       Microservice: Beckn BAP Client (8002)
                      └── seller_offerings  (uuid-E) ← POST /normalize/discovery (one per provider)
                      │                                 N rows — one per offering found
                            └── scored_offers  (uuid-F)        ← POST /normalize/scoring
                            │                                     Microservice: Comparative Scoring (8003)
                                  └── negotiation_outcomes  (uuid-G)  ← POST /normalize/order
                                  │                                      Phase 3 placeholder (strategy=skipped)
                                        └── approval_decisions  (uuid-H) ← POST /normalize/order
                                        │                                   Phase 3 placeholder (auto_approved)
                                              └── purchase_orders  (uuid-I) ← POST /normalize/order
                                                                               Microservice: Beckn BAP Client /select (8002)
```

### Microservice → Table(s) Mapping

| Microservice | Port | Tables it generates |
|---|---|---|
| Orchestrator (entry) | 8004 | `procurement_requests` |
| Intention Parser | 8001 | `parsed_intents`, `beckn_intents` |
| Beckn BAP Client `/discover` | 8002 | `discovery_queries`, `seller_offerings`, `bpp` (upsert) |
| Comparative Scoring | 8003 | `scored_offers` |
| Beckn BAP Client `/select` | 8002 | `negotiation_outcomes`, `approval_decisions`, `purchase_orders` |

### Why the tree order matters

Each child table **cannot exist without its parent** — the DB rejects the INSERT if the parent UUID does not exist. This means that if a `_persist()` call fails, all child nodes from that level downward will never be created, even though the business pipeline continues working normally for the user.
...
via `_persist()` after each pipeline step.

```python
DATA_NORMALIZER_URL = os.getenv("DATA_NORMALIZER_URL", "http://localhost:8006")
```

### Call pattern: best-effort with retries

`_persist()` implements a **best-effort** pattern — if the Data Normalizer does not respond, the business pipeline is never blocked. From the current implementation:

- **3 retries** with linear backoff: waits 1s between attempt 1→2, 2s between attempt 2→3
- **5s timeout** per attempt
- **Never raises an exception** — errors are logged as `WARNING` and the function returns `{}`
- If all 3 attempts fail, `"audit record lost"` is logged and the pipeline continues

```python
# Current behavior in workflow.py
for attempt in range(1, 4):          # 3 attempts
    try:
        async with session.post(..., timeout=5s) as resp:
            if resp.status < 300:
                return await resp.json()
            logger.warning("... HTTP %s (attempt %s/3)", resp.status, attempt)
    except Exception as exc:
        logger.warning("... failed (attempt %s/3): %s", attempt, exc)
    if attempt < 3:
        await asyncio.sleep(attempt)  # 1s, then 2s
logger.warning("... gave up after 3 attempts — audit record lost")
return {}
```

### Risk: silent chain collapse

The audit tree IDs are generated by the DB (via `gen_random_uuid()`) and returned by the Data Normalizer. The Orchestrator uses those IDs to chain subsequent calls:

```
_persist("/normalize/request") → fails → request_id = ""
    → if request_id: does NOT enter → /normalize/intent is NEVER called
        → beckn_intent_id = "" → /normalize/discovery is NEVER called
            → ... the entire chain collapses silently
```

**Consequence:** If all 3 retries fail on the first call, the entire audit tree remains empty in the DB — but the user receives their results normally.

### When this risk occurs

| Cause | Probability | Symptom in logs |
|---|---|---|
| Data Normalizer down | Low (Docker compose) | `WARNING: gave up after 3 attempts` |
| DB under high load (>5s per query) | Medium | `WARNING: failed ... asyncio.TimeoutError` |
| 5xx error from normalizer | Low | `WARNING: returned HTTP 500` |
| Slow Docker network | Very low | `WARNING: failed ... ClientConnectorError` |

### Future improvements (Phase 3)

| Improvement | Description | Impact |
|---|---|---|
| UUIDs in Orchestrator | Generate UUIDs before `_persist()`, remove return dependency | Eliminates chain collapse |
| Message queue (Redis/RabbitMQ) | Guaranteed audit delivery even when normalizer is down | 100% reliable audit |
...
> Tests in this section cover **only the path the browser takes**: Step 1 (direct call to intention-parser) and Steps 2→3→4 (via orchestrator). The Data Normalizer is called automatically by the orchestrator after each step — the frontend never calls it directly.
...
FROM beckn_intents
ORDER BY delivery_timeline_hours DESC LIMIT 1;
...
ORDER BY sq.queried_at DESC LIMIT 1;
...
**Goal:** The browser picks the top-ranked offering and calls `orchestrator:8004/commit`. The orchestrator confirms with Beckn and persists the full FK chain: `negotiation_outcomes → approval_decisions → purchase_orders`.
...
**Goal:** Verify that if the Data Normalizer is down, the frontend flow still returns live data (no crash). Persistence is best-effort — the orchestrator never fails because of it.
